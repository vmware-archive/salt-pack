...
{{name}}-{{version.replace('.', '_')}}:
  pkgbuild.built:
    ...
    - results: 
      ...
      - {{pypi_alt_name}}_{{version}}-{{release_ver}}.dsc
      - {{pypi_alt_name}}_{{version}}-{{release_ver}}.diff.gz
    - force: {{force}}
    - dest_dir: {{buildcfg.build_dest_dir}}
    - spec: salt://{{slspath}}/spec/{{pypi_alt_name}}_{{version}}-{{release_ver}}.dsc 
    - tgt: {{buildcfg.build_tgt}}
    - template: jinja
    - sources:
      - salt://{{slspath}}/sources/{{pypi_alt_name}}_{{version}}.orig.tar.gz
      - salt://{{slspath}}/sources/{{pypi_alt_name}}_{{version}}-{{release_ver}}.diff.gz
