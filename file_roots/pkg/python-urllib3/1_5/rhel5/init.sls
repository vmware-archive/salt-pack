...
{{ macros.requires(sls_name, pkg_data) }}
    ...
    - sources:
      ...
      - salt://{{slspath}}/sources/{{sls_name}}-unbundle.patch
      - salt://{{slspath}}/sources/{{sls_name}}-accept-header-for-proxy.patch
      - salt://{{slspath}}/sources/{{sls_name}}-default-ssl-cert-validate-el5.patch
{% endif %}
