...
{{name}}-{{version.replace('.', '_')}}:
  pkgbuild.built:
    ...
    - results: 
      ...
      - {{pypi_name}}_{{version}}-{{release_ver}}{{release_nameadd}}.tar.gz
    - force: {{force}}
    ...
    - sources:
      ...
      - salt://{{slspath}}/sources/{{pypi_name}}_{{version}}-{{release_ver}}{{release_nameadd}}.tar.gz
